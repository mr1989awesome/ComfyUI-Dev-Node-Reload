...
LOADED_MODULE_DIRS = {}
CUSTOM_NODE_PACK_NODE_IDS = {}
CUSTOM_NODE_PACK_DISPLAY_NAMES = {}
CUSTOM_NODE_PACK_SYS_MODULES = {}
CUSTOM_NODE_PACK_EXTENSION_DIR_KEYS = {}


def _module_is_within_directory(module, directory: str) -> bool:
    module_file = getattr(module, "__file__", None)
    if module_file is None:
        return False
    try:
        return folder_paths.is_within_directory(directory, os.path.abspath(module_file))
    except Exception:
        return False


def _get_pack_owned_sys_modules(module_dir: str):
    owned = set()
    for sys_module_name, module in list(sys.modules.items()):
        if module is None:
            continue
        if _module_is_within_directory(module, module_dir):
            owned.add(sys_module_name)
    return owned


def _get_custom_node_root_for_path(module_dir: str):
    module_dir = os.path.abspath(module_dir)
    for custom_node_root in folder_paths.get_folder_paths("custom_nodes"):
        custom_node_root = os.path.abspath(custom_node_root)
        if folder_paths.is_within_directory(custom_node_root, module_dir):
            return custom_node_root
    return None


def _remove_pack_registrations(module_name: str):
    for node_id in CUSTOM_NODE_PACK_NODE_IDS.get(module_name, set()):
        NODE_CLASS_MAPPINGS.pop(node_id, None)
    for node_id in CUSTOM_NODE_PACK_DISPLAY_NAMES.get(module_name, set()):
        NODE_DISPLAY_NAME_MAPPINGS.pop(node_id, None)
    for ext_key in CUSTOM_NODE_PACK_EXTENSION_DIR_KEYS.get(module_name, set()):
        EXTENSION_WEB_DIRS.pop(ext_key, None)


def _restore_pack_registrations(module_name: str, previous_state):
    NODE_CLASS_MAPPINGS.update(previous_state["node_mappings"])
    NODE_DISPLAY_NAME_MAPPINGS.update(previous_state["display_name_mappings"])
    for ext_key, ext_dir in previous_state["extension_dirs"].items():
        EXTENSION_WEB_DIRS[ext_key] = ext_dir
    if previous_state["module_dir"] is not None:
        LOADED_MODULE_DIRS[module_name] = previous_state["module_dir"]
    CUSTOM_NODE_PACK_NODE_IDS[module_name] = set(previous_state["node_mappings"].keys())
    CUSTOM_NODE_PACK_DISPLAY_NAMES[module_name] = set(previous_state["display_name_mappings"].keys())
    CUSTOM_NODE_PACK_SYS_MODULES[module_name] = set(previous_state["owned_sys_modules"])
    CUSTOM_NODE_PACK_EXTENSION_DIR_KEYS[module_name] = set(previous_state["extension_dirs"].keys())


def get_dev_reload_allowlist():
    return set(args.dev_reload_node_pack)
...
async def reload_custom_node(module_name: str):
    if not args.enable_dev_node_reload:
        return {
            "success": False,
            "error": "dev_reload_disabled",
            "message": "Development-only custom node reload is disabled.",
            "restart_required": False,
        }

    if module_name not in get_dev_reload_allowlist():
        return {
            "success": False,
            "error": "pack_not_allowlisted",
            "message": f"Custom node pack '{module_name}' is not allowlisted for development reload.",
            "restart_required": False,
        }

    module_dir = LOADED_MODULE_DIRS.get(module_name)
    if module_dir is None:
        return {
            "success": False,
            "error": "pack_not_loaded",
            "message": f"Custom node pack '{module_name}' is not currently loaded.",
            "restart_required": False,
        }

    if _get_custom_node_root_for_path(module_dir) is None:
        return {
            "success": False,
            "error": "pack_outside_custom_nodes",
            "message": f"Custom node pack '{module_name}' is not loaded from a configured custom_nodes folder.",
            "restart_required": True,
        }

    previous_state = {
        "node_mappings": {node_id: NODE_CLASS_MAPPINGS[node_id] for node_id in CUSTOM_NODE_PACK_NODE_IDS.get(module_name, set()) if node_id in NODE_CLASS_MAPPINGS},
        "display_name_mappings": {node_id: NODE_DISPLAY_NAME_MAPPINGS[node_id] for node_id in CUSTOM_NODE_PACK_DISPLAY_NAMES.get(module_name, set()) if node_id in NODE_DISPLAY_NAME_MAPPINGS},
        "extension_dirs": {ext_key: EXTENSION_WEB_DIRS[ext_key] for ext_key in CUSTOM_NODE_PACK_EXTENSION_DIR_KEYS.get(module_name, set()) if ext_key in EXTENSION_WEB_DIRS},
        "owned_sys_modules": set(CUSTOM_NODE_PACK_SYS_MODULES.get(module_name, set())),
        "module_dir": module_dir,
    }

    ignore = set(NODE_CLASS_MAPPINGS.keys()) - set(previous_state["node_mappings"].keys())

    _remove_pack_registrations(module_name)
    LOADED_MODULE_DIRS.pop(module_name, None)
    for sys_module_name in previous_state["owned_sys_modules"]:
        sys.modules.pop(sys_module_name, None)

    importlib.invalidate_caches()
    success = await load_custom_node(module_dir, ignore=ignore, module_parent="custom_nodes")
    if not success:
        _restore_pack_registrations(module_name, previous_state)
        return {
            "success": False,
            "error": "reload_failed",
            "message": f"Failed to reload custom node pack '{module_name}'. Full restart required.",
            "restart_required": True,
        }

    new_extension_dirs = {
        ext_key: EXTENSION_WEB_DIRS[ext_key]
        for ext_key in CUSTOM_NODE_PACK_EXTENSION_DIR_KEYS.get(module_name, set())
        if ext_key in EXTENSION_WEB_DIRS
    }
    if new_extension_dirs != previous_state["extension_dirs"]:
        _remove_pack_registrations(module_name)
        _restore_pack_registrations(module_name, previous_state)
        return {
            "success": False,
            "error": "web_directory_changed",
            "message": f"Custom node pack '{module_name}' changed its registered web directories and requires a full restart.",
            "restart_required": True,
        }

    current_node_ids = set(CUSTOM_NODE_PACK_NODE_IDS.get(module_name, set()))
    previous_node_ids = set(previous_state["node_mappings"].keys())
    return {
        "success": True,
        "message": f"Reloaded custom node pack '{module_name}'.",
        "restart_required": False,
        "pack": module_name,
        "node_ids": sorted(current_node_ids),
        "added_node_ids": sorted(current_node_ids - previous_node_ids),
        "removed_node_ids": sorted(previous_node_ids - current_node_ids),
    }
...
async def load_custom_node(module_path: str, ignore=set(), module_parent="custom_nodes") -> bool:
    ...
    try:
        ...
        existing_extension_keys = set(EXTENSION_WEB_DIRS.keys())
        ...
        registered_node_ids = set()
        registered_display_names = set()
        ...
        if hasattr(module, "NODE_CLASS_MAPPINGS") and getattr(module, "NODE_CLASS_MAPPINGS") is not None:
            ...
            if hasattr(module, "NODE_DISPLAY_NAME_MAPPINGS") and getattr(module, "NODE_DISPLAY_NAME_MAPPINGS") is not None:
                for name, display_name in module.NODE_DISPLAY_NAME_MAPPINGS.items():
                    ...
                    registered_display_names.add(name)
            if module_parent == "custom_nodes" and os.path.isdir(module_dir):
                CUSTOM_NODE_PACK_NODE_IDS[module_name] = registered_node_ids
                CUSTOM_NODE_PACK_DISPLAY_NAMES[module_name] = registered_display_names
                CUSTOM_NODE_PACK_SYS_MODULES[module_name] = _get_pack_owned_sys_modules(module_dir)
                CUSTOM_NODE_PACK_EXTENSION_DIR_KEYS[module_name] = set(EXTENSION_WEB_DIRS.keys()) - existing_extension_keys
            return True
            ...
            try:
                ...
                for node_cls in node_list:
                    ...
                    if schema.node_id not in ignore:
                        ...
                        node_cls.RELATIVE_PYTHON_MODULE = "{}.{}".format(module_parent, get_module_name(module_path))
                        registered_node_ids.add(schema.node_id)
                    if schema.display_name is not None:
                        NODE_DISPLAY_NAME_MAPPINGS[schema.node_id] = schema.display_name
                        registered_display_names.add(schema.node_id)
                if module_parent == "custom_nodes" and os.path.isdir(module_dir):
                    CUSTOM_NODE_PACK_NODE_IDS[module_name] = registered_node_ids
                    CUSTOM_NODE_PACK_DISPLAY_NAMES[module_name] = registered_display_names
                    CUSTOM_NODE_PACK_SYS_MODULES[module_name] = _get_pack_owned_sys_modules(module_dir)
