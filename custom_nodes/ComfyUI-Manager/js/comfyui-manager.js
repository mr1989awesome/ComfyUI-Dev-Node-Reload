...
import {
	...
	rebootAPI, setManagerInstance, show_message, customAlert, customPrompt, customConfirm,
...
class DevReloadManagerDialog extends ComfyDialog {
	constructor() {
		super();
		this.devReloadFeature = {
			enabled: false,
			packs: []
		};
		this.reloadingDevPack = null;
		this.statusElement = $el("div.cm-dev-reload-status", {
			textContent: "Loading local dev packs..."
		});
		this.listElement = $el("div.cm-dev-reload-list");

		const content = $el("div.dr-manager-body", {}, [
			$el("div.dr-manager-note", {
				textContent: "Reload allowlisted local custom node packs without restarting the ComfyUI server."
			}),
			$el("div.cm-dev-reload-panel", {}, [
				this.statusElement,
				this.listElement
			])
		]);
	...
	async loadDevReloadFeature() {
		try {
			...
			const config = features?.dev_custom_node_reload;
			if (!config) {
				return;
			}
			...
			console.warn("[ComfyUI-Manager] Failed to load dev reload feature state.", error);
		}
	}

	renderPanel() {
		if (!this.devReloadFeature.enabled) {
			this.statusElement.textContent = "Dev reload is disabled.";
			this.listElement.replaceChildren($el("div.cm-dev-reload-name", {
				textContent: "Start ComfyUI with --enable-dev-node-reload to use node reload manager."
			}));
			return;
		}

		if (!this.devReloadFeature.packs.length) {
			this.statusElement.textContent = "No allowlisted local dev packs.";
			this.listElement.replaceChildren($el("div.cm-dev-reload-name", {
				textContent: "Add packs with --dev-reload-node-pack to list them here."
			}));
			return;
		}

		this.statusElement.textContent = "Reload allowlisted local custom node packs.";
		const rows = this.devReloadFeature.packs.map((pack) => {
			const button = $el("button.p-button.p-component.cm-button.cm-small-button.cm-dev-reload-button", {
				type: "button",
				textContent: this.reloadingDevPack === pack ? "Reloading..." : "Reload",
				disabled: this.reloadingDevPack !== null,
				onclick: () => this.reloadDevNodePack(pack)
			});

			return $el("div.cm-dev-reload-row", {}, [
				$el("div.cm-dev-reload-name", {
					textContent: pack,
					title: pack
				}),
				button
	...
	async reloadDevNodePack(pack) {
		if (this.reloadingDevPack !== null) {
			return;
		}

		this.reloadingDevPack = pack;
		this.statusElement.textContent = `Reloading ${pack} ...`;
		this.renderPanel();

		try {
			const response = await api.fetchApi("/api/dev/reload_custom_node", {
				method: "POST",
				headers: {
			...
			if (!response.ok) {
				const errorMessage = payload?.message || payload?.error || `Failed to reload ${pack}.`;
				this.statusElement.textContent = errorMessage;
				customAlert(errorMessage);
				return;
			}

			this.statusElement.textContent = `Reloaded ${pack}.`;
			infoToast(`Reloaded ${pack}. Refresh the browser to load updated node definitions.`);

			const confirmed = await customConfirm(`'${pack}' reloaded successfully. Refresh the browser now to pick up node definition changes?`);
			if (confirmed) {
				window.location.reload();
			}
		} catch (error) {
			const errorMessage = error?.message || `Failed to reload ${pack}.`;
			this.statusElement.textContent = errorMessage;
			customAlert(errorMessage);
		} finally {
			this.reloadingDevPack = null;
...
app.registerExtension({
	...
	{
		id: "Comfy.Manager.DevReload.ToggleVisibility",
		label: "Toggle Node Reload Manager Visibility",
		icon: "mdi mdi-refresh",
		function: () => {
			if (!DevReloadManagerDialog.instance) {
				DevReloadManagerDialog.instance = new DevReloadManagerDialog();
				DevReloadManagerDialog.instance.show();
			} else {
				DevReloadManagerDialog.instance.toggleVisibility();
			}
		},
	},
	{
	...
	async setup() {
		...
		try {
			...
			let cmGroup = new (await import("../../scripts/ui/components/buttonGroup.js")).ComfyButtonGroup(
				new(await import("../../scripts/ui/components/button.js")).ComfyButton({
					...
					tooltip: "Node Reload Manager",
					content: "Reload Nodes",
		...
		const reloadManagerButton = document.createElement("button");
		reloadManagerButton.textContent = "Reload Nodes";
		reloadManagerButton.onclick = () => {
			if (!DevReloadManagerDialog.instance) {
				DevReloadManagerDialog.instance = new DevReloadManagerDialog();
			}
			DevReloadManagerDialog.instance.show();
		}
		menu.append(reloadManagerButton);
