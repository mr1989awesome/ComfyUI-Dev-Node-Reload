...
class PromptServer():
    def __init__(self, loop):
        ...
        self.dev_reload_lock = asyncio.Lock()
        self.dev_reload_in_progress = False
        ...
        async def get_features(request):
            ...
            features["dev_custom_node_reload"] = {
                "enabled": bool(args.enable_dev_node_reload),
                "packs": sorted(nodes.get_dev_reload_allowlist()),
            }
        ...
        async def post_prompt(request):
            if self.dev_reload_in_progress:
                error = {
                    "type": "dev_reload_in_progress",
                    "message": "Custom node reload in progress",
                    "details": "Prompt submission is temporarily disabled while a development-only custom node reload is running.",
                    "extra_info": {}
                }
        ...
        @routes.post("/dev/reload_custom_node")
        async def post_dev_reload_custom_node(request):
            json_data = await request.json()
            module_name = json_data.get("pack")
            if not isinstance(module_name, str) or len(module_name.strip()) == 0:
                return web.json_response({
                    "success": False,
                    "error": "missing_pack",
                    "message": "Request body must include a non-empty 'pack' string.",
                    "restart_required": False,
                }, status=400)

            result = await self.reload_dev_custom_node(module_name.strip())
            status = 200 if result.get("success") else 409
            if result.get("error") in {"dev_reload_disabled", "pack_not_allowlisted"}:
                status = 403
            elif result.get("error") == "pack_not_loaded":
                status = 404
    ...
    async def reload_dev_custom_node(self, module_name: str):
        if self.dev_reload_lock.locked():
            return {
                "success": False,
                "error": "reload_busy",
                "message": "Another custom node reload is already in progress.",
                "restart_required": False,
            }
        ...
        if self.prompt_queue.get_tasks_remaining() != 0:
            return {
                "success": False,
                "error": "queue_not_idle",
                "message": "Custom node reload requires an idle queue. Wait for running or pending prompts to finish first.",
                "restart_required": False,
            }

        async with self.dev_reload_lock:
            self.dev_reload_in_progress = True
            try:
                if self.prompt_queue.get_tasks_remaining() != 0:
                    return {
                        "success": False,
                ...
                result = await nodes.reload_custom_node(module_name)
            finally:
                self.dev_reload_in_progress = False

        event_name = "custom_nodes_reloaded" if result.get("success") else "custom_nodes_reload_failed"
        self.send_sync(event_name, result)
        return result
